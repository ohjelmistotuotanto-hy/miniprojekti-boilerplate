...
Esimerkissä on käytössä projekti `toska-playground`. Jokaiselle ohtuprojektiryhmälle on luotu oma, muista ryhmistä eristetty OpenShift-projekti (namespace). Kysy ohjaajaltasi ryhmäsi namespacen nimi.
...
Namespace on tässä tapauksessa _toska-playground_, se vastaa OpenShift-projektin nimeä, eli ohtuprojekteilla oman ryhmän namespacea. Host-nimen pitää olla Tiken klusteritasolla uniikki, sopiva nimi on esim. sovelluksen nimi ja sen perässä namespacen nimi. `spec/to` määrittelee reitityksen kohteena olevan palvelun. Kohdeportiksi pitää määritellä servicen takana olevan podin portti, ei siis servicen portti (joka oli tapauksessamme 80), servicen sisäistä porttia käytetään tapauksessamme klusterin sisäisessä kommunikoinnissa.
...
Jokaisella ryhmällä on oma namespace, joten eri ryhmien objektit eivät voi törmätä nimiavaruudessa keskenään. Silti kannattaa nimetä oman namespacen sisäiset Kubernetes-objektit järkevästi ja yhdenmukaisesti, jotta esim. `oc get all` -komennon tuloste pysyy luettavana, varsinkin jos namespaceen päätyy useampia sovelluksia (esim. backend ja tietokanta). Jos sovelluksen nimi olisi esim. labtool, objektien nimet voisivat noudattaa seuraavaa kaavaa
...
Klusterille ei saa jättää mitään roskaa eikä ylimääräisiä Kubernetes-objekteja. Eli jos jokin resurssi todetaan turhaksi, se poistetaan.
